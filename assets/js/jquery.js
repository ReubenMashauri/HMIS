...
// http://weblogs.java.net/blog/driscoll/archive/2009/09/08/eval-javascript-globalcontext
...
booleans =
"checked|selected|async|autofocus|autoplay|controls|defer|disabled|hidden|ismap|loop|multipl
e|open|readonly|required|scoped",
// Regular expressions
...
// An unquoted value should be a CSS identifier http://www.w3.org/TR/css3-
...
// Operator (capture 2)
"*([*^$|!~]?=)" + whitespace +
// "Attribute values must be CSS identifiers [capture 5] or strings [capture
3 or capture 4]"
"*(?:'((?:\\\\.|[^\\\\'])*)'|\"((?:\\\\.|[^\\\\\"])*)\"|(" + identifier +
"))|)" + whitespace +
"*\\]",
...
reloaded,
// IE will throw "permission denied" error when accessing "document" variable, see
jQuery #13936
...
---------------------------------------------------------------------- */
// Support: IE<8
// Verify that getAttribute really returns attributes and not properties (excepting
IE8 booleans)
...
div.innerHTML = "<select msallowclip=''><option selected=''>
</option></select>";
...
library/ie/hh465388.aspx#attribute_section
...
"string" && elem.className || typeof elem.getAttribute !== strundefined &&
...
on `parent`
if ( forward && useCache ) {
// Seek `elem` from a previouslycached
index
...
expando ] || (elem[ expando ] = {}))[ type ]) && cache[0] === dirruns ) {
diff = cache[1];
// xml :nth-child(...) or :nth-lastchild(...)
or :nth(-last)?-of-type(...)
} else {
...
if ( useCache ) {
(node[
expando ] || (node[ expando ] = {}))[ type ] = [ dirruns, diff ];
}
if ( node === elem )
{
break;
...
// Reuse newcache so results backpropagate
...
// The foundational matcher ensures that elements are reachable from toplevel
context(s)
...
] for forwarding actions to newDefer
...
support[ i + "Bubbles" ] = div.attributes[ eventName ].expando ===
false;
}
}
// Null elements to avoid leaks in IE.
...
mouseHooks: {
props: "button buttons clientX clientY fromElement offsetX offsetY pageX
...
body && body.scrollLeft || 0 ) - ( doc && doc.clientLeft || body && body.clientLeft || 0 );
event.pageY = original.clientY + ( doc && doc.scrollTop ||
body && body.scrollTop || 0 ) - ( doc && doc.clientTop || body && body.clientTop || 0 );
}
...
// Support: IE<9
// If we error on focus to hidden element
(#1486, #12518),
...
// Allow triggered, simulated change events
(#11500)
...
undefined;
if ( !found ) {
for ( found = [], elems = context.childNodes || context; (elem = elems[i])
!= null; i++ ) {
...
elem.childNodes[j]), "tbody" ) && !tbody.childNodes.length ) {
...
// Remove cache only if it was not already removed
by jQuery.event.remove
if ( cache[ id ] ) {
delete cache[ id ];
...
if ( node.src ) {
// Optional AJAX dependency,
but won't run scripts if not present
if ( jQuery._evalUrl ) {
jQuery._evalUrl(
node.src );
}
...
"-webkit-box-sizing:content-box;-moz-box-sizing:contentbox;"
+
...
if ( window.getComputedStyle ) {
pixelPositionVal = ( window.getComputedStyle( div, null ) || {}
).top !== "1%";
...
// swappable if display is none or starts with table except "table", "table-cell",
or "table-caption"
...
isBorderBox = support.boxSizing && jQuery.css( elem, "boxSizing", false,
styles ) === "border-box";
...
name,
extra,
support.boxSizing && jQuery.css( elem, "boxSizing",
...
// WebKit Bug 13343 - getComputedStyle returns wrong value for
margin-right
// Work around by temporarily setting element display to inlineblock
...
show and we are going to proceed with show, we should pretend to be hidden
...
while ( i-- ) {
option = options[ i ];
if ( jQuery.inArray( jQuery.valHooks.option.get(
option ), values ) >= 0 ) {
// Support: IE6
// When new option element is added to
select box we need to
// force reflow of newly added node in order
to workaround delay
...
// http://fluidproject.org/blog/2008/01/09/getting-settingand-
removing-tabindex-values-with-javascript/
...
var rquery = (/\?/);
var rvalidtokens = /(,)|(\[|{)|(}|])|"(?:[^"\\\r\n]|\\["\\\/bfnrt]|\\u[\da-fA-F]{4})*"\s*:?
|true|false|null|-?(?!0\d)\d+(?:\.\d+|)(?:[eE][+-]?\d+|)/g;
...
in a way that preserves old ones
statusCode[ code ] = [
...
// See http://msdn.microsoft.com/enus/
library/ie/ms536648(v=vs.85).aspx
...
already provided.
if ( !options.crossDomain && !headers["X-Requested-
With"] ) {
...
for ( i in headers ) {
// Support: IE<9
// IE's ActiveXObject throws a 'Type
Mismatch' exception when setting
// request header to a null-value.
//
// To keep consistent with other XHR
implementations, cast the value
...
// We normalize with
Webkit giving an empty statusText
statusText = "";
}
// Filter status for non
standard behaviors
// If the request is local
and we have data: assume a success
// (success with no data
won't get notified, that's the best we
// can do given current
implementations)
if ( !status &&
options.isLocal && !options.crossDomain ) {
status =
responses.text ? 200 : 404;
// IE - #1450: sometimes
returns 1223 when it should be 204
} else if ( status === 1223
) {
status = 204;
}
...
// data: string of html
// context (optional): If specified, the fragment will be created in this context, defaults
to document
...
offset[Width/Height] or client[Width/Height], whichever is greatest
// unfortunately, this causes bug #3838 in IE6/8
only, but there is currently no good, small way to fix it.
return Math.max(
